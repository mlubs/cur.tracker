...
(() => {
  ...
  function plotParityChart(data) {
    ...
    // Paridade EURBRL x USDBRL (primeiro eixo) - Mostra rela̤̣o entre EUR e USD em BRL
    const eurUsdParity = data.map(row => {
      if (row.USDBRL && row.EURBRL && row.USDBRL !== 0) {
